(function () {
  var root = document.getElementById('amo-tasks');
  var browser = document.getElementById('amo-tasks-browser');
  var gridEl = document.getElementById('amo-tasks-grid');
  var sidebarEl = document.getElementById('amo-tasks-sidebar');
  var titleEl = document.getElementById('amo-tasks-detail-title');
  var bodyEl = document.getElementById('amo-tasks-detail-body');
  var backBtn = document.getElementById('amo-tasks-back-grid');
  var sourceEl = document.getElementById('amo-tasks-source');
  ...
  function getTopics() {
    return Array.prototype.slice.call(sourceEl.querySelectorAll('section[data-topic-id]')).map(function (section) {
      var content = section.querySelector('.amo-tasks-topic');
      return {
  ...
  function setActiveMarkers(topicId) {
    sidebarEl.querySelectorAll('.amo-tasks-browser__sidebar-item').forEach(function (item) {
      item.classList.toggle('help-sidebar__item--active', item.dataset.topicId === topicId);
    });
    gridEl.querySelectorAll('.amo-tasks-browser__topic').forEach(function (item) {
  ...
  function buildGrid() {
    ...
    getTopics().forEach(function (topic) {
      ...
      li.className = 'help-topic__item help-topic__item--clickable amo-tasks-browser__topic';
  ...
  function buildSidebar() {
    ...
    getTopics().forEach(function (topic) {
      var li = document.createElement('li');
      li.className = 'help-sidebar__item amo-tasks-browser__sidebar-item';
      li.dataset.topicId = topic.id;
